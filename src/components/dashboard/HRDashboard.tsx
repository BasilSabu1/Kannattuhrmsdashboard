...
const HRDashboard = () => {
  ...
  const handleStatusUpdate = async (uuid: string, newStatus: string) => {
    try {
      ...
      const response = await axiosInstance.patch(API_URLS.USERS.PATCH_USER_BY_UUID(uuid), {
        status: newStatus
      });
        ...
        <CardContent>
          <div className="space-y-4">
            {applications.map((application) => (
              <div key={application.uuid} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{application.education_employment?.designation || 'N/A'} • {application.education_employment?.branch || 'N/A'}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <p className="text-sm text-muted-foreground">Documents</p>
                    <p className="font-medium">{application.document_uploads?.images.length || 0}/8</p>
                  </div>
                  ...
                  applications.map((app) => (
                    <TableRow key={app.uuid}>
                      ...
                      <TableCell>
                        <Select
                          value={app.status}
                          onValueChange={(value) => handleStatusUpdate(app.uuid, value)}
